...
abstract class CSSRuleSet
{
	...
	*/
	public function getRules($mRule = null)
	{
		...
		} elseif (isset($this->aRules[$mRule])) {
	...
	*/
	  ...
	  'thin', 'medium', 'thick'
		...
		foreach ($aBorderRules as $sBorderRule) {
			...
			foreach ($aValues as $mValue) {
				...
				} elseif ($mValue instanceof CSSColor) {
	...
	public function expandFontShorthand()
	{
		...
		foreach ($aValues as $mValue) {
			...
			} elseif ($mValue instanceof CSSRuleValueList && $mValue->getListSeparator() == '/') {
				list($oSize, $oHeight) = $mValue->getListComponents();
				$aFontProperties['font-size'] = $oSize;
				$aFontProperties['line-height'] = $oHeight;
			} elseif ($mValue instanceof CSSSize && $mValue->getUnit() !== null) {
	...
	public function expandBackgroundShorthand()
	{
	  ...
	  'background-position' => [new CSSSize(0, '%'), new CSSSize(0, '%')]
	];
		$mRuleValue = $oRule->getValue();
		$aValues = [];
		if (!$mRuleValue instanceof CSSRuleValueList) {
