	...
	public function __construct($mColor = null)
	{
		...
		if (is_array($mColor)) {
			if (isset($mColor['r'], $mColor['g'], $mColor['b'])) {
				$this->fromRGB($mColor);
			} elseif (isset($mColor['h'], $mColor['s'], $mColor['l'])) {
				$this->fromHSL($mColor);
			}
		} elseif (is_string($mColor)) {
			if ($aRGB = CSSColorUtils::namedColor2rgb($mColor)) {
				$this->fromRGB($aRGB);
			} elseif ($aRGB = CSSColorUtils::hex2rgb($mColor)) {
				$this->fromRGB($aRGB);
			}
		}
	}
