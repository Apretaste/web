   ...
   **/
	...
	public function testCssFilesProvider()
	{
		...
		if ($rHandle = opendir($sDirectory)) {
			...
			while (false !== ($sFileName = readdir($rHandle))) {
				...
				$aFiles[] = [$sDirectory.DIRECTORY_SEPARATOR.$sFileName, ''];
			}
			closedir($rHandle);
		}
		return $aFiles;
	...
	*/
	...
	public function testSpecificity()
	{
		...
		$this->assertEquals([new CSSSelector('#test .help', true)], $oDoc->getSelectorsBySpecificity('> 100'));
	}
	...
	public function testFunctionSyntax()
	{
		...
		$sExpected = str_replace(['1.2em', '0.2em', '60%'], ['3.6em', '0.6em', '180%'], $sExpected);
		$this->assertSame($sExpected, $oDoc->__toString());
		...
		$sExpected = str_replace(['0.2s', '0.3s', '90deg'], ['0.4s', '0.6s', '180deg'], $sExpected);
		$this->assertSame($sExpected, $oDoc->__toString());
	}
	...
	public function parsedStructureForFile($sFileName)
	{
