<?php
require_once(dirname(__FILE__).'/lib/CSSCharsetUtils.php');
require_once(dirname(__FILE__).'/lib/CSSUrlUtils.php');
require_once dirname(__FILE__).'/lib/CSSColorUtils.php';
require_once(dirname(__FILE__).'/lib/CSSProperties.php');
require_once(dirname(__FILE__).'/lib/CSSList.php');
require_once(dirname(__FILE__).'/lib/CSSRuleSet.php');
require_once(dirname(__FILE__).'/lib/CSSRule.php');
require_once(dirname(__FILE__).'/lib/CSSValue.php');
require_once(dirname(__FILE__).'/lib/CSSValueList.php');
...
class CSSParser
{
	 ...
	 **/
	public function parseFile($sPath, $aLoadedFiles = [])
	{
	 ...
	 * @return CSSDocument the resulting CSSDocument
	 **/
	public function parseURL($sPath, $aLoadedFiles = [])
	{
		if (!$this->getOption('base_url')) {
	...
	private function postParse($oDoc)
	{
		...
		foreach ($aContents as $i => $oItem) {
			if ($oItem instanceof CSSIgnoredValue) {
				unset($aContents[$i]);
			} elseif ($oItem instanceof CSSCharset) {
				$aCharsets[] = $oItem;
				unset($aContents[$i]);
			} elseif ($oItem instanceof CSSImport) {
				$aImports[] = $oItem;
				unset($aContents[$i]);
			}
		}
		$aImportedItems = [];
		$aImportOptions = array_merge($this->getOptions(), [
	  ...
	  'base_url' => null
		...
		foreach ($aImports as $oImport) {
			if ($this->getOption('resolve_imports')) {
				...
				} elseif ($this->sImportMode == self::IMPORT_FILE) {
					$sPath = realpath($sPath);
					if (!in_array($sPath, $this->aLoadedFiles)) {
				...
				if ($oImport->getMediaQuery() !== null) {
					...
					$aImportedContents = [$oMediaQuery];
				}
			} else {
				$aImportedContents = [$oImport];
			}
	...
	private function parseRule()
	{
		...
		if ($this->comes('!')) {
			...
			if (mb_convert_case($sImportantMarker, MB_CASE_LOWER) !== 'important') {
				throw new Exception("! was followed by â€œ".$sImportantMarker."â€�. Expected â€œimportantâ€�");
			}
	...
	private function parseMSFilter()
	{
		...
		$this->consume('(');
		$oValue = new CSSFunction($sName, $this->parseValue(['=', ',']));
		$this->consume(')');
		if ($bAsString) {
			$this->consumeWhiteSpace();
			$this->consume($this->comes('"') ? '"' : "'");
	...
	private function parsePrimitiveValue()
	{
		...
		} elseif ($this->comes('#') || $this->comes('rgb') || $this->comes('hsl')) {
			$oValue = $this->parseColorValue();
		} elseif ($this->comes('url')) {
